...
## blackfire    :	Access `blackfire` using its container.
blackfire:
	docker exec -ti -e COLUMNS=$(shell tput cols) -e LINES=$(shell tput lines) $(shell docker ps --filter name='$(PROJECT_NAME)_blackfire' --format "{{ .ID }}") blackfire $(filter-out $@,$(MAKECMDGOALS))
